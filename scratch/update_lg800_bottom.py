...
backup_path = os.path.join(backup_dir, '鑄件盤點資料_backup_before_lg800_update.xlsx')
...
for row in range(2, ws.max_row + 1):
    ...
    if model_val is not None:
        ...
        if model_str.upper() == target_model.upper():
            ...
            print(f"Updated row {row}: Model='{model_val}', Old Part Number={old_part_number} -> New Part Number={new_part_number}")
            found = True
            break
